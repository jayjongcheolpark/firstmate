...
The tracked Claude hook entries whose event Grok already covers through its own `.grok/hooks/` registration run only where `CLAUDECODE` is present and no `GROK_AGENT` or `GROK_HOOK_EVENT` is, because Grok also loads Claude-compatible project settings and otherwise creates a second blocking path; the exact marker set, why `GROK_SESSION_ID` is excluded, and the per-hook-kind measurement the positive condition rests on are owned by `docs/turnend-guard.md` "Harness integrations".
