...
Every treehouse-backed spawn (tmux included) requires `jq` to read `treehouse status --json` before allocating a pool slot; the allocation rule itself is owned by the `bin/fm-spawn.sh` header.
